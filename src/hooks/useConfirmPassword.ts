...
export default function useConfirmPassword() {
  ...
  const setAndVerifyPassword: ChangeEventHandler<HTMLInputElement> =
    useCallback(
      (event) => {
        ...
        const textVerify = isIncludeAlphabet(passwordText);
        if (!textVerify) {
          setPasswordVerifyMessage("문자를 1개 이상 포함해야 합니다.");
          setPasswordVerify(false);
          return;
        }
        const numberVerify = isIncludeNumber(passwordText);
        if (!numberVerify) {
          setPasswordVerifyMessage("숫자를 1개 이상 포함해야 합니다.");
          setPasswordVerify(false);
          return;
        }
        const lengthVerify = isLongerThanNumber(passwordText, 8);
        if (!lengthVerify) {
  ...
  useEffect(() => {
    ...
    const confirmPasswordLonger = isLongerThanNumber(confirmPassword, 8);
